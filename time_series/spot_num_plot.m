...
figure(4);
ssn = d(:,3); % Extract SSN column.
% Create matrix with 11*12 rows and 24 columns.  Columns correspond to 
% cycle and rows correspond to month in cycle.
ssnr = reshape(ssn(1:24*11*12),12*11,24);
imagesc(ssnr);
ch = colorbar;
th = get(ch,'Title');
set(gca,'YTick',[0:12:11*12]); % Use sensible positions.
set(gca,'YTickLabels',[0:12:11*12]/12); % Use sensible labels.
set(th,'String','SSN'); % Searched on "Colorbar title" to figure out how to do this.
%ylabel('Month in 11-year Cycle');
ylabel('Year in 11-year Cycle');
xlabel('Cycle Number');
print -depsc spot_num_ssn_cycle.eps
print -dpng spot_num_ssn_cycle.png

break

% Testing what I would expect if I had a signal with a 11-year cycle.
figure(5)
t = [1:24*11*12];
x = sin(2*pi*t/(11*12));
